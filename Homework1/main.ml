...
A.Axes.box part2;
A.Viewport.yrange part2 (-.1.) 2.;
A.Viewport.xlabel part2 "Time (Gigayears)";
A.Viewport.ylabel part2 "g-r";
A.fx part2 Util.gr_at_time 0. 10.;
A.close part2;;

let mag_to_gr (t : float) = (Util.gr_at_time t, Util.stellar_mag_at_time t);;
...
for i = 0 to 5 do 
    gr.(i) <- Util.gr_at_time (t_arr.(i));
    mag.(i) <- Util.stellar_mag_at_time (t_arr.(i));
done;

let part3 = A.init ["Cairo";"PNG";"MagToG_R.png"] in 
A.Axes.box part3; 
A.Viewport.xrange part3 (-.1.) 1.;
A.Viewport.yrange part3 0. 10.;
